...
if [[ $1 = "date" ]];then
    dunstify -h string:bgcolor:#000000 -h string:x-dunst-stack-tag:date "$(date "+%a %h-%m %d %H:%M:%S") mem: $(free -m | grep Mem: | awk '{print $3}') cpu_temp: $(sensors | tail -n 2 | sed 's/[^0-9.°C]//g')"
    exit
fi
